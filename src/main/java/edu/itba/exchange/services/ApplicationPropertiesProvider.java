...
public record ApplicationPropertiesProvider(Properties properties) implements PropertiesProvider {
    ...
    public ApplicationPropertiesProvider{try(
    final var in = new FileInputStream(FILE_PATH)){properties.load(in);}catch(
    IOException e){throw new RuntimeException(FILE_PATH+" not found!");}}
